...
function LanguageSwitcherInner({ variant = "dropdown", className }: LanguageSwitcherProps) {
  ...
  return (
    <div ref={rootRef} className={cn("relative", className)}>
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className={cn(
          "inline-flex items-center gap-2 rounded-full border border-white/10 bg-background/60 px-3 py-2",
          "font-display text-[10px] uppercase tracking-widest text-foreground backdrop-blur-sm",
          "transition hover:border-neon-green/50 hover:text-neon-green",
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={cn(
              "absolute right-0 top-full z-[70] mt-2 w-56 overflow-hidden rounded-xl",
              "border border-white/10 bg-[oklch(0.11_0_0/0.96)] shadow-[0_24px_80px_-20px_rgba(0,0,0,0.8)]",
              "backdrop-blur-xl",
            )}
            role="listbox"
            aria-label={t("languageSwitcher.label")}
